...
const IRRIGATION_METHODS = ['Drip', 'Sprinkler', 'Flood', 'Furrow', 'Manual', 'Rainfall'];
...
export default function MaintenanceScreen() {
  ...
  const [herbicideUsed,     setHerbicideUsed]     = useState('');
  const [irrigationMethod,  setIrrigationMethod]  = useState('');
  const [weedingCost,       setWeedingCost]       = useState('');
  ...
  function validate(): boolean {
    ...
    if (!fertilizerType.trim() && !pesticideUsed.trim() && !herbicideUsed.trim()) {
      e.fertilizerType = 'Enter at least one maintenance activity';
    }
    setErrors(e);
  ...
  function handleSave() {
    ...
    setTimeout(() => {
      setSavedLogs(prev => [{
        ...
        fertilizerType, fertilizerQty, pesticideUsed,
        herbicideUsed, irrigationMethod, weedingCost,
  ...
  function resetForm() {
    setFertilizerType(''); setFertilizerQty(''); setPesticideUsed('');
    setHerbicideUsed(''); setIrrigationMethod(''); setWeedingCost('');
    setErrors({});
  }
    ...
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Crop Maintenance</Text>
          <Text style={styles.headerSub}>Stage 3 of 5</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 120 }]} showsVerticalScrollIndicator={false}>
        <Card style={styles.infoCard}>
          <View style={styles.infoRow}>
            <View style={styles.infoIconCircle}>
              <MaterialIcons name="eco" size={22} color="#1A7A4A" />
            </View>
            <View style={styles.infoText}>
              <Text style={styles.infoTitle}>What to log here</Text>
              <Text style={styles.infoBody}>Fertilizer, pesticide, herbicide, irrigation method and weeding costs.</Text>
            </View>
          </View>
        </Card>
            ...
            {savedLogs.map(log => (
              <Card key={log.id} style={styles.logCard}>
                <View style={styles.logRow}>
                  ...
                  <Text style={styles.logText}>{log.fertilizerType || 'Maintenance'} · {log.irrigationMethod || 'No irrigation'}</Text>
        ...
        {savedLogs.length === 0 && (
          <Card style={styles.emptyCard}>
            <MaterialIcons name="eco" size={40} color="#E0E0E0" />
            <Text style={styles.emptyTitle}>No records yet</Text>
            <Text style={styles.emptySubtitle}>Tap the button below to log maintenance</Text>
          </Card>
        )}

        <Button label="+ Log Maintenance" onPress={() => setModalVisible(true)} />
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalSheet}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Maintenance Log</Text>
              <TouchableOpacity onPress={() => { setModalVisible(false); resetForm(); }}>
                <MaterialIcons name="close" size={24} color="#9CA3AF" />
              </TouchableOpacity>
            </View>
            <ScrollView contentContainerStyle={styles.modalContent}>
              <Input label="Fertilizer Type" value={fertilizerType} onChangeText={setFertilizerType} placeholder="e.g. NPK, Urea" error={errors.fertilizerType} />
              ...
              <Input label="Pesticide Used" value={pesticideUsed} onChangeText={setPesticideUsed} placeholder="e.g. Cypermethrin" />
              <Input label="Herbicide Used" value={herbicideUsed} onChangeText={setHerbicideUsed} placeholder="e.g. Glyphosate" />
              <Text style={styles.inputLabel}>Irrigation Method</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
                {IRRIGATION_METHODS.map(m => (
                  <TouchableOpacity
                    key={m}
                    onPress={() => setIrrigationMethod(irrigationMethod === m ? '' : m)}
                    style={[styles.chip, irrigationMethod === m && styles.chipActive]}
                  >
                    <Text style={[styles.chipText, irrigationMethod === m && styles.chipTextActive]}>{m}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
              <Input label="Weeding Labour Cost (GHS)" value={weedingCost} onChangeText={setWeedingCost} keyboardType="decimal-pad" placeholder="e.g. 120" />
              <Button label="Save Maintenance" onPress={handleSave} loading={loading} />
